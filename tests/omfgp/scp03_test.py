...
def test_derive_data():
    ...
    context = bytes.fromhex("68E9C8799FF8CB46F8D89D88BA1287BF")
    s_enc = _derive_data(key, DDC.S_ENC, len(key) * 8, context)
    assert s_enc == bytes.fromhex("3C64070C595D1D72BDEE692DA99B7AE4")
    s_mac = _derive_data(key, DDC.S_MAC, len(key) * 8, context)
    assert s_mac == bytes.fromhex("6E8468B74A707842E3D3696F338D2657")
    s_rmac = _derive_data(key, DDC.S_RMAC, len(key) * 8, context)
    assert s_rmac == bytes.fromhex("FDD1D7C84750994815D0082734C31D88")
    card_cryptogram = _derive_data(s_mac, DDC.CARD_CRYPTOGRAM, 0x0040, context)
    assert card_cryptogram == bytes.fromhex("9B5C9D42D5059430")
    host_cryptogram = _derive_data(s_mac, DDC.HOST_CRYPTOGRAM, 0x0040, context)
    assert host_cryptogram == bytes.fromhex("F3C7624565B5894F")
