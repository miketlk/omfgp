...
def test_scp03_clear(mocker: MockerFixture):
    ...
    fake_card = FakeCard({
        INITIALIZE_UPDATE + b'\x00\x00': (
            bytes.fromhex("2E6805A2847B9844"),
            bytes.fromhex("00008048007283073469FF0300E4A2A0"
    ...
    assert (scp.wrap_apdu(GET_STATUS + b'\x80\x02' + card.encode("4F00")) ==
            bytes.fromhex("80F28002024F00"))
    response = (bytes.fromhex("E32A4F08A0000001510000009F700107"
...
def test_scp03_C_MAC(mocker: MockerFixture):
    ...
    fake_card = FakeCard({
        INITIALIZE_UPDATE + b'\x00\x00': (
            bytes.fromhex("68E9C8799FF8CB46"),
            bytes.fromhex("00008048007283073469FF0300F8D89D"
                          "88BA1287BF9B5C9D42D5059430"), SUCCESS),
        EXTERNAL_AUTHENTICATE + b'\x01\x00': (
            bytes.fromhex("F3C7624565B5894F16EF5681573C34A2"), b'', SUCCESS)
    }, debug=True)
    progress_cb = mocker.stub()
    scp = open_secure_channel(
    ...
    # Wrap GET STATUS (0x80, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x80\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F280020A4F004C4C51B5DFA7A181"))
    response = (bytes.fromhex("E32A4F08A0000001510000009F700107"
                              "C5039EFE80C407A0000000620001CE02"
                              "0100CC08A000000151000000"), SUCCESS)
    assert scp.unwrap_response(*response) == response
    ...
    # Wrap GET STATUS (0x40, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x40\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F240020A4F002108D3E0BA43B673"))
    response = (b'', b'\x6A\x88')
    assert scp.unwrap_response(*response) == response
...
def test_scp03_C_DECRYPTION_C_MAC(mocker: MockerFixture):
    ...
    fake_card = FakeCard({
        INITIALIZE_UPDATE + b'\x00\x00': (
            bytes.fromhex("CC8A0407F63066E8"),
            bytes.fromhex("00008048007283073469FF0300EF23AA"
                          "B4C343EDDE5B283674A8983749"), SUCCESS),
        EXTERNAL_AUTHENTICATE + b'\x03\x00': (
            bytes.fromhex("112355D07180C24547A5EB3BD470F7C5"), b'', SUCCESS)
    }, debug=True)
    progress_cb = mocker.stub()
    scp = open_secure_channel(
    ...
    # Wrap GET STATUS (0x80, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x80\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F28002184EE4C1127567676E7E0883"
    ...
    # Wrap GET STATUS (0x40, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x40\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F2400218E33CB507A6FFF09CE7A44E"
...
def test_scp03_C_MAC_R_MAC(mocker: MockerFixture):
    ...
    fake_card = FakeCard({
        INITIALIZE_UPDATE + b'\x00\x00': (
            bytes.fromhex("CB6DA009E0404A67"),
            bytes.fromhex("0000013628140599538231037021428D"
                          "A9468C2D5A8D05CE9657FBD5ED00001D"), SUCCESS),
        EXTERNAL_AUTHENTICATE + b'\x11\x00': (
            bytes.fromhex("CC606C998B98BF8AF0746E4F50D3F65E"), b'', SUCCESS)
    }, debug=True)
    progress_cb = mocker.stub()
    scp = open_secure_channel(
...
def test_scp03_C_DECRYPTION_C_MAC_R_MAC(mocker: MockerFixture):
    ...
    fake_card = FakeCard({
        INITIALIZE_UPDATE + b'\x00\x00': (
            bytes.fromhex("F70E1C5D67085E4B"),
            bytes.fromhex("000001362814059953823103707B9EAD"
                          "94A8B88028F47137A2FB093F5E000021"), SUCCESS),
        EXTERNAL_AUTHENTICATE + b'\x13\x00': (
            bytes.fromhex("2E2057B873FCD25D175D4D63C5CD4CEB"), b'', SUCCESS)
    }, debug=True)
    progress_cb = mocker.stub()
    scp = open_secure_channel(
    ...
    # Wrap GET STATUS (0x80, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x80\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F2800218E9170B7D79EA80AF507247"
    ...
    # Wrap GET STATUS (0x40, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x40\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F240021804B77FFA640854E3BA8611"
    ...
    # Wrap GET STATUS (0x10, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x10\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F2100218291B45B303C43B6222834B"
...
def test_scp03_C_DECRYPTION_R_ENCRYPTION_C_MAC_R_MAC(mocker: MockerFixture):
    ...
    fake_card = FakeCard({
        INITIALIZE_UPDATE + b'\x00\x00': (
            bytes.fromhex("0C980267C298D194"),
            bytes.fromhex("00000136281405995382310370730BE0"
                          "14C03B4558A2155798D5B2C602000023"), SUCCESS),
        EXTERNAL_AUTHENTICATE + b'\x33\x00': (
            bytes.fromhex("9716680D938E0CE764738F77E5E39408"), b'', SUCCESS)
    }, debug=True)
    progress_cb = mocker.stub()
    scp = open_secure_channel(
    ...
    # Wrap GET STATUS (0x80, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x80\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F2800218AFB12C966FF538690FA041"
    ...
    # Wrap GET STATUS (0x40, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x40\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F240021851EF5596042E756C67E7A0"
    ...
    # Wrap GET STATUS (0x10, 0x02) and unwrap response
    assert (scp.wrap_apdu(GET_STATUS + b'\x10\x02' + card.encode("4F00")) ==
            bytes.fromhex("84F2100218CEB683FB79244285024670"
...
def test_scp03_Key_DEK():
    ...
    ct = bytes.fromhex("014E02F68B094BE48EE05AFC1B1A14D6"
